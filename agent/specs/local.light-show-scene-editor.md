...
- **R5.** `scenes.list` MUST return scenes as `{scenes: [...], total: number, has_more: boolean}` where `scenes[]` rows have shape `{id, label, type, params, created_at, updated_at}`. The `params` field MUST be the **sparse stored value** (only keys explicitly overridden by the user) — NOT merged with catalog defaults. Catalog defaults are resolved at evaluator time, not at list time, so list → set round-trips do not promote defaults to explicit overrides. It MUST accept optional args:
  ...
  - `filter.label_query: string` — substring match on label, **ASCII case-insensitive** (`LOWER(label) LIKE '%' || LOWER(query) || '%'` semantics). Limitation: SQLite's built-in `LOWER()` and `LIKE` only handle A-Z↔a-z mapping — non-ASCII characters (accented Latin, CJK, Cyrillic, etc.) are compared case-sensitively. Acceptable for MVP since scene labels are typically English / ASCII. Full-Unicode case-insensitivity is a forward-compat upgrade path: register a Python custom function (`conn.create_function("LOWER_UNI", 1, str.lower, deterministic=True)`) and swap the SQL, OR migrate to FTS5 with the `unicode61` tokenizer (phase-2 search upgrade documented in Non-Goals). Neither change affects the MCP tool surface — only the storage backend behind `label_query`.
