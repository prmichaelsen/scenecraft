...
import { getClipColorScheme } from '@/lib/clip-colors'
...
function AudioClipBlock({ projectName, clip, pxPerSec, laneHeight, isInMultiSelect, selectedIds, onClipClick, onRequestAlignWaveforms, onRequestDeleteClip, onRequestToggleMute, onClipMouseDown, onClipTrimMouseDown, dragOffsetPx = 0, dragOffsetPy = 0, trimPreview, isHighlighted = false }: AudioClipBlockProps) {
  ...
  const colors = useMemo(() => getClipColorScheme(clip.variant_kind), [clip.variant_kind])
  return (
    <div
      className={`absolute top-1 bottom-1 rounded-sm overflow-hidden border ${colors.bg} ${colors.bgHover} ${isDragging ? '' : 'transition-colors'} ${
        selected
          ? `${colors.borderSelected} ring-1 ${colors.ring}`
          : showHighlight
