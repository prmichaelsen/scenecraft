...
export async function fetchNarrative(project: string) {
  const res = await fetch(`${SCENECRAFT_API_URL}/api/projects/${encodeURIComponent(project)}/narrative`)
...
export async function fetchTimelines(project: string) {
  const res = await fetch(`${SCENECRAFT_API_URL}/api/projects/${encodeURIComponent(project)}/timelines`)
...
export async function postSwitchTimeline(project: string, name: string) {
  const res = await fetch(`${SCENECRAFT_API_URL}/api/projects/${encodeURIComponent(project)}/timeline/switch`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function postCreateTimeline(project: string, name: string, copyFrom?: string) {
  const res = await fetch(`${SCENECRAFT_API_URL}/api/projects/${encodeURIComponent(project)}/timeline/create`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function postImportTimeline(project: string, sourcePath: string, timelineName?: string) {
  const res = await fetch(`${SCENECRAFT_API_URL}/api/projects/${encodeURIComponent(project)}/timeline/import`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
